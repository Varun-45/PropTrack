...
export const userotpaction = (entereddata) => async (dispatch) => {
  try {
    dispatch({ type: USER_OTP_REQUEST });
    const config = { headers: { "Content-Type": "application/json" } };
    
    // console.log('data')
    const {data} = await axios.post("/api/v1/otp/generate", entereddata, { config });
    
    dispatch({
      type: USER_OTP_SUCCESS,
      payload: data.useralready,
    });
  } catch (error) {
    dispatch({
      type: USER_OTP_FAIL,
...
//   ADMIN 
  ...
  // Time Elapsed by user (Not Completed)
